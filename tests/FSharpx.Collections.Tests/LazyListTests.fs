...
module LazyList =
    ...
    let testLazyList =
        ...
        testList "LazyList" [
            ...
            test "se2" {Expect.isFalse "cons" <| LazyList.isEmpty (LazyList.cons 1 LazyList.empty) }
            test "se3" {Expect.isFalse "repeat" <| LazyList.isEmpty (LazyList.repeat 1) }
            test "se4" {Expect.isFalse "unfold" <|  LazyList.isEmpty (LazyList.unfold (fun z -> Some (z,z+1)) 0) }

            test "seq1" {Expect.equal "cons" 1 <| LazyList.head (LazyList.cons 1 LazyList.empty) }
            test "seq2" {Expect.equal "cons" 1 <| LazyList.head (LazyList.cons 1 (LazyList.cons 2 LazyList.empty)) }
            test "seq3" {Expect.equal "cons" 2 <| LazyList.head (LazyList.tail (LazyList.cons 1 (LazyList.cons 2 LazyList.empty))) }
            ...
            test "tryHead seq2" {Expect.equal "tryHead" 1 (LazyList.tryHead (LazyList.cons 1 (LazyList.cons 2 LazyList.empty))).Value }

            test "tryTail empty" {Expect.isNone "tryTail" (LazyList.tryTail LazyList.empty) }
            test "tryTail seq1" {Expect.isTrue "tryTail" <| LazyList.isEmpty (LazyList.tryTail (LazyList.cons 1 LazyList.empty)).Value}
            ...
            test "fold" {
                let ll = LazyList.ofList [-5..-1]
                let expected = 15
                Expect.equal "fold" expected <| LazyList.fold (fun a b -> a + abs b) 0 ll }

            test "repeat" {Expect.equal "take repeat" [1;1;1;1] <| LazyList.toList (LazyList.take 4 (LazyList.repeat 1)) }
            test "append" {Expect.equal "append"  [77;0;1;2] <| LazyList.toList (LazyList.take 4 (LazyList.append (LazyList.cons 77 (LazyList.empty)) nats)) }
            test "zip"  {Expect.equal "zip" [0,6;1,7; 2,8] <| LazyList.toList (LazyList.take 3 (LazyList.zip nats (LazyList.skip 6 nats))) }
            test "firstS" {Expect.equal "tryFind" (Some 8) <| LazyList.tryFind (fun x -> x>=8) nats }
            test "firstN" {Expect.isNone "take" <| LazyList.tryFind (fun x -> x>=8) (LazyList.take 5 nats) }
            test "find S" {Expect.equal "find" 8 <| LazyList.find (fun x -> x>=8) nats }

            test "find N" {Expect.throwsT<KeyNotFoundException> "find N" (fun () -> LazyList.find (fun x -> x>=8) (LazyList.take 5 nats) |> ignore) }

            test "consfA" {Expect.equal "consDelayed" 1 <| LazyList.head (LazyList.consDelayed 1 diverge) }
            test "consfB"       {Expect.isTrue "divergence" (let ss = LazyList.tail (LazyList.consDelayed 1 diverge) in true) } (* testing for lazy divergence *)
            ...
            test "takedrop" {Expect.equal "takedrop" [4;5;6] <| LazyList.toList (LazyList.take 3 (LazyList.skip 4 nats)) }
            test "filter" {Expect.equal "filter" [0;2;4;6] <| LazyList.toList (LazyList.take 4 (LazyList.filter (fun x -> x % 2 = 0) nats)) }
